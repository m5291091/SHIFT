...
from .models import Member, ShiftPattern, LeaveRequest, TimeSlotRequirement, Assignment, DayGroup
...
def generate_schedule(start_date_str, end_date_str):
    ...
    for m in all_members:
        for d in days:
            for p in all_patterns:
                ...
                total_priority_score.append(shifts[(m.id, d, p.id)] * priority_reward * work_minutes)
            ...
            for p in all_patterns:
                ...
                for i in range(num_intervals):
                    ...
                    if start_date <= slot_start_dt.date() <= end_date:
                        slot_coverage[(slot_start_dt.date(), slot_start_dt.time())].append(shifts[(m.id, d, p.id)])
    for d in days:
        ...
        for t in range(0, 24 * 60, time_interval):
            ...
            if rule_for_slot:
                workers_in_slot = slot_coverage.get((d, current_slot_start), [])
    ...
    for m in all_members:
        ...
        if max_work_days >= 0:
            model.Add(sum(work_days_in_period) <= max_work_days)
            ...
            for d in days:
                for p in all_patterns:
                    if (m.id, d, p.id) in shifts and solver.Value(shifts[(m.id, d, p.id)]) == 1:
                        Assignment.objects.create(member_id=m.id, shift_pattern_id=p.id, shift_date=d)
        return True
    return False